...
long DrvObject::textureMemoryUsed() const
{
	return 0;
}

void DrvObject::resetAllDeviceObjects()
{
	...
	while ( item )
	{
		DrvObject* nextItem = item->m_next;
		item->resetDeviceObject();
		item = nextItem;
	}
}

void DrvObject::destroyAllDeviceObjects()
{
	synchronized( s_objects );
	...
	{
		DrvObject* nextItem = item->m_next;
		item->destroyDeviceObject();
		item = nextItem;
	}
}

long DrvObject::totalTextureMemoryUsed()
{
	synchronized( s_objects );
	...
	while ( item )
	{
		texMemUsed += item->textureMemoryUsed();
		item = item->m_next;
	}
