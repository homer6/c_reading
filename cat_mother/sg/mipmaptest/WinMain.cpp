...
int WINAPI WinMain( HINSTANCE inst, HINSTANCE, LPSTR cmdLine, int cmdShow )
{
	...
	try
	{
		...
		lt->setName( "keylight" );
		lt->setIntensity( 0.8f );
		lt->linkTo( root );
		lt->lookAt( Vector3(-0.5f,-1,0.5f) );

		// resources
		P(Texture) envmap = loadTex( "envmap.dds" );
		P(CubeTexture) difmap = loadCubeTex( "TestCubeTexture2.dds" );
		...
		P(Model) model = new Model( vertices, indices, vf );
		{
			...
			for ( int i = 0 ; i < vertices ; ++i )
			{
				...
				model->setVertexTextureCoordinates( i, 0, 3, (float*)(vbbytes.begin()+12+i*vertexSize) );
			}
	...
	// env transform 2d
		...
		mat->setTextureCoordinateTransform( 0, Material::TTFF_COUNT3, Matrix4x4(1) );
		mat->setTextureCoordinateSource( 0, Material::TCS_VERTEXDATA);

		mat->setTexture( 1, envmap );
		mat->setTextureColorCombine( 1, Material::TA_TEXTURE, Material::TOP_ADD, Material::TA_CURRENT );		
		mat->setTextureCoordinateTransform( 1, Material::TTFF_COUNT2, textm2d );
		...
		for ( int i = 0 ; i < keys ; ++i )
		{
			...
			float v[3] = { Math::sin(f*Math::PI*2.f)*2.f, 0, Math::sin(f*Math::PI*2.f)*6.f + 2.f };
			anim->setKeyValue( i, v, 3 );
			anim->setKeyTime( i, f * 8.f );
		}
		anim->sortKeys();
