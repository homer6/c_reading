...
void QuaternionUtil::slerp( float* d, float t, const float* p, const float* q )
{
	float cos = dot(p,q);
...
void QuaternionUtil::normalize( float* d, const float* q )
{
...
float QuaternionUtil::norm( const float* q )
{
	return sqrtf( dot(q,q) );
}

void QuaternionUtil::inverse( float* d, const float* q )
{
	float n = norm(q);
...
void QuaternionUtil::exp( float* d, const float* q )
{
	assert( d != q );
...
void QuaternionUtil::log( float* d, const float* q )
{
	assert( d != q );
...
void QuaternionUtil::mul( float* d, const float* a, const float* b )
{
	assert( d != a && d != b );
...
void QuaternionUtil::squad( float* d, float t, const float* p, const float* a, const float* b, const float* q )
{
	float ptq[4];
	float atb[4];
	slerp( ptq, t, p, q );
