...
{
	const String commonPrefix = "c:/projects/";
	...
	for ( int i = 0 ; i < stats.size() ; ++i )
	{
		...
		if ( bytes == 0 )
			bytes = 1;
		Debug::println( "  {0}: uses {5,#.#}%, {1,#} bytes in {2,#} blocks, total {3,#} bytes in {4,#} blocks", name, mg.bytesInUse, mg.blocksInUse, mg.bytesTotal, mg.blocksTotal, (float)mg.bytesInUse/(float)bytes*100.f );
	}
}
